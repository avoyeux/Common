...
class Decorators:
    ...
    @staticmethod  
    def running_time(verbose_name: F | str = 'verbose', flush_name: str = 'flush') -> Callable[[F], F]:
        """
        ...
        # Check if running_time is used without (), i.e. the function is passed directly without arguments or with an empty ()
    ...
    def print_inputs(
        ...
        """
        To print the argument names and information of a decorated function. You can also choose what should be printed.

        Args:
            defaults (F | bool, optional): When decorator arguments are given, it lets you decide if the default arguments that 
                weren't called should be printed. If no arguments are set, it points to the decorated function just like a decorator would.
                Defaults to False.
            ...
            shape (bool, optional): deciding to print the shape of the argument. It outputs len() if the argument isn't a dict or a ndarray.
                Defaults to True.
            size (bool, optional): deciding to print the size of the argument. Only work if the argument is a ndarray. Defaults to False.
            flush (bool, optional): deciding to force the internal buffer to immediately write the output to it's destination, i.e. it decides to
                force the prints or not. Has a negative effect on the running efficiency as you are forcing the buffer but makes sure that the print 
                is outputted exactly when it is called (usually not the case when multiprocessing). Defaults to False.
        ...
        if callable(defaults):
            func = defaults
            defaults = False
            return DecoratorsUtils.actual_print_inputs_decorator(func, defaults, type_, value, shape, size, flush)
...
class DecoratorsUtils:
    """
    Stores private functions that are used to help the added functionalities in this Decorators class.
    """
    ...
    def actual_running_time_decorator(func: F, verbose_name: str, flush_name: str) -> F:
        ...
        def wrapper(*args: any, **kwargs: any) -> any:
            ...
            try:
                ...
                raise Exception(f"\033[1;31mFunction {func.__name__} didn't run properly. The corresponding error is: {e}\033[0m")
            finally:
                END_time = time()
