...
Only small changes were applied to better correspond to my usage and code style (I don't really
follow the PEP 8 guidelines...). Furthermore. some functions that I don't need were taken out.
This code is actually one of the first 'proper' codes that I have used and the basics for the
Common repository I have created.
...
class SpiceUtils:
    """
    Stores some useful functions to access and manipulate the SPICE catalogue and corresponding
    filenames.
    """
    ...
    def read_spice_uio_catalog(verbose: int = 0, flush: bool = False) -> pd.DataFrame:
        """
        Read csv table SPICE FITS files catalog. Works on the server or locally if the
        ~/.ssh/config is properly set-up using a key and 'sol' as the ssh connection shortcut
        (c.f. Common.ServerConnection.ServerUtils.ssh_connect()).
        ...
        * `df[(df.LEVEL == "L2") \
        ...
        Source:
        https://spice-wiki.ias.u-psud.fr/doku.php/data:data_analysis_manual:read_catalog_python
        """
        ...
        df.LEVEL = df.LEVEL.apply(
            lambda string: string.strip() if isinstance(string, str) else string
        )
        df.STUDYTYP = df.STUDYTYP.apply(
            lambda string: string.strip() if isinstance(string, str) else string
        )
        return df
...
def get_mosaic_filenames(verbose: int = 0, flush: bool = False) -> list[str]:
    ...
    Args:
        verbose (int, optional): defines the level of the prints. 0 means none and the higher the
            more low level are the prints. Defaults to 0.
        flush (bool, optional): sets the internal buffer to immediately write the output to it's
            destination, i.e. it decides to force the prints or not. Has a negative effect on the
            running efficiency as you are forcing the buffer but makes sure that the print is
            outputted exactly when it is called (usually not the case when multiprocessing).
            Defaults to False.
